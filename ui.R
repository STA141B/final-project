...
shinyUI(
  fluidPage(
    navbarPage("Coronavirus(COVID-19) Real-time Dynamics",
      theme = shinytheme("slate"),
      tabPanel("World Trend",
        fluid = TRUE, icon = icon("globe"), # page 1
        titlePanel("Culmulative Confirmed Cases and Deaths"),
        br(),
        # function to run a spinner when app is loading in the begining
        add_busy_spinner(
      ...
      tabPanel("Detailed Situation",
        fluid = TRUE, icon = icon("table"), # page 2
      ...
      tabPanel("Typical Country Data",
        icon = icon("location-arrow"), # page 4
      ...
      tabPanel("Breaking News",
        icon = icon("bolt"), # page 5
      ...
      navbarMenu("More Information",
        icon = icon("bullhorn"), # page 6 to show navagate bar
        tabPanel("General Q&A",
        ...
        "----",
        tabPanel("U.S. Testing",
          icon = icon("flask"), # bolt flask
          titlePanel("Daily Testing by State around U.S."),
          mainPanel(
            offset = 4, width = 8,
            br(),
            tabsetPanel(
              tabPanel("Current U.S. Testing",
                icon = icon("ambulance"),
                br(),
                br(),
        ...
        "----",
        tabPanel("About",
          icon = icon("info-circle"), # bolt flask
