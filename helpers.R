...
rename_countries <- function(data){
  ...
  data[, 1][data[, 1] == "Ivory Coast", ] = "Céte d'Ivoire"
...
read_coro_data <- function(url, world){
  ...
  colnames(data) <- title
  data[, 2:length(title)] <- apply(data[, 2:length(title)], 2, as.numeric)
...
Draw_world_map <- function(data_Map, geo_data){
  ...
  bins <- c(0,100,1000,5000,10000,30000,80000,100000,200000)
  bins2 <- c(0,5,10,50,100,500,1000,3000,10000,20000)
...
max_death <- max(country_data$TotalDeaths, na.rm = T)
  ...
  if (country_name == "China"){
    ...
    bins_country <- c(0,10,50,100,1000,max_case)
    bins_country2 <- c(0,1,5,10,max_death)
  } else if (country_name == "Australia"){
    bins_country <- c(0,50,100,500,1000,max_case)
    bins_country2 <- c(0,1,5,max_death)
  }
...
USmap <- function(url){
  ...
  bins_US <- c(0,100,500,1000,2000,5000,10000,max_US)
